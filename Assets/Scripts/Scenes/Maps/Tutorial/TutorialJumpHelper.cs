...
public class TutorialJumpHelper : MonoBehaviour
{
    ...
    private IEnumerator OnTriggerEnter( Collider other )
    {
        if(!alreadyTriggered)
        {
            if(other.tag == CommonGameObjectsTags.PLAYER_TAG)
            {
                ...
                yield return StartCoroutine( dialogueManager.StartDialogue( DialogueText.TUTORIAL_JUMP_HELPER ) );

                BotGenerator botGenerator = FindObjectOfType<BotGenerator>();
                botGenerator.PlayCustomRecording( CustomRecordingsNames.TUTORIAL_SCENE_TUTORIAL_JUMP_HELPER, ResumePlay );
